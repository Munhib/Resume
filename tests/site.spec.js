...
test('pages, skill filters, project controls, and downloads work',async({page,request})=>{
 ...
 await page.screenshot({path:'test-results/projects-desktop.png',fullPage:false});
 ...
 await page.getByRole('link',{name:'Contact',exact:true}).click();await expect(page.locator('.email-link')).toHaveAttribute('href','mailto:munhibbaig@gmail.com');await page.reload();await expect(page.locator('#contact .section-title')).toBeInViewport();
...
test('mobile navigation and pages fit the viewport',async({page})=>{
 ...
 for(const name of ['About','Stack','Projects','Contact','Home']){await page.getByRole('button',{name:'Toggle navigation'}).click();await page.getByRole('link',{name,exact:true}).click();await expect(page.locator('#'+name.toLowerCase())).toBeInViewport();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBeTruthy();}
 await page.screenshot({path:'test-results/home-mobile.png',fullPage:false});
});

test('scrolling reaches every section and updates navigation without clicks',async({page})=>{
 await page.goto('/');
 const canvas=page.locator('.landscape canvas');
 const original=await canvas.elementHandle();
 for(const name of ['About','Stack','Projects','Contact']){
   await page.evaluate(id=>document.getElementById(id).scrollIntoView(),name.toLowerCase());
